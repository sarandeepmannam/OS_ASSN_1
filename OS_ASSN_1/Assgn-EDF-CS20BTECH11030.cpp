...
process p;
// function to sort an array of processes according to their deadlines
...
// function that gives the available highest priority process (process that have nearest dead line). If no process available it returns a invalid process with pid=-1
process more_priority(process pro[],int n,int time)
{
  ...
  for(int i=0;i<n;i++)
  {
    if(p1.ndl==pro[i].ndl && p1.pid>pro[i].pid && pro[i].nst<=time &&  pro[i].count>0)
    {
    p1=pro[i];
    }
  }
...
// Function to find the slot of the process with given pid in an array of processes.
int find(int pid,process pro[],int n)
{
 for(int i=0;i<n;i++)
 {
   if(p.pid==pro[i].pid)
...
{ 
  ...
  fscanf(fp,"%d",&n);
  int total=0;//total no of iterations of all processes
  process pro[n];// array to store all the processes
  ...
  fclose(fp);
  fp1=fopen("EDF-log.txt","w");
  //Initialising all processes
   for(int i=0;i<n;i++)
  {
  ...
  while(check_complete(pro,n)!=1)
   {
    ...
    for(int i=0;i<n;i++)
    {
    int h=find(p.pid,pro,n);
    if(time>pro[i].nst && i!=h && pro[i].count>0)
    {
      pro[i].wait_time++;
    }
    }
    ...
    if(time+p.time_left>p.ndl)
    {
     ...
     else 
     {
      ...
      else
      {
       p=p1;
       int k=p.priority;
       fprintf(fp1,"p1rocesss %d resumes at time %d\n",p.pid,time);
       pro[k-1].time_left--;
       p.time_left--;
      }
     }
     ...
     else if(p1.pid!=p.pid)
     {
      if(p.time_left==p.cpu_burst )
      {
       ...
       else
       {
        pro[k-1].time_left--;
        p.time_left--;
        fprintf(fp1,"p2rocess %d resumes execution at time %d\n" ,p.pid,time);
       }
      }
      else
      {  
       ...
       else
       {
        pro[k-1].time_left--;
        p.time_left--;
        fprintf(fp1,"p3rocess %d resumes execution at time %d\n",p.pid,time);
       }
      }
     }
    else
